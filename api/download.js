...
// Add page only when content won't fit — generous bottom margin
function safeY(doc, needed = 40) {
  if (doc.y + needed > doc.page.height - 55) doc.addPage();
}

// Footer written directly after content — never jumps to page.height
...
// Strip junk chars — normalize bullet lines from any source
function cleanLine(raw) {
  // Remove common PDF-extraction artifacts: %¸ %· %, and other non-ASCII bullet encodings
  return raw
    .replace(/^[\s]*[%][¸·,.\-•►▸*○●◆◦‣⁃→\s]+/, "")  // %¸ style
    .replace(/^[\s]*[•►▸◦‣⁃○●◆→]+\s*/, "")             // unicode bullets
    .replace(/^[\s]*[-–—]\s+/, "")                       // dash bullets
    .trim();
}
...
function buildResumePDF(doc, optimizedText, photo, candidateName) {
  ...
  // Gather contact lines (lines after name before first blank or body)
  let contactParts = [];
  let bodyStart = 1;
  ...
  // Divider drawn BELOW photo — never intersects it
  ...
  for (const line of body) {
    ...
    // Job/company line (contains separator but no @)
...
function buildCoverPDF(doc, coverLetter, candidateName) {
  ...
  // Strip junk from AI output: repeated names, [placeholders], contact lines
...
function buildReportPDF(doc, report, candidateName) {
  ...
  const STOP = new Set(["job","description","the","and","or","with","for","to","of",
    ...
    "which","who","when","where","ensure","looking","need","must","should","can","may",
    "able","work","working","role","position","team","using","use","used","experience",
    "company","including","strong","good","excellent","apply","responsible","candidate",
    "ability","skills","great","well","new","best","please","would","could","also",
    "want","require","required","preferred","applications","validating","bugs","will"]);
